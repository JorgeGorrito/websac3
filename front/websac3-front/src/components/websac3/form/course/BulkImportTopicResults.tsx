...
import type { BulkImportTopicResult } from "@/services/api";

interface BulkImportTopicResultsProps {
  result: BulkImportTopicResult;
  onClose: () => void;
}

export function BulkImportTopicResults({ result, onClose }: BulkImportTopicResultsProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 pr-8 mb-6">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Resultados de la Importación de Tópicos</h2>
          ...
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2 text-green-700">
                ...
                Tópicos Importados Exitosamente ({result.successful_count})
              </CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {result.successful_items.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div>
                      ...
                      <div className="text-sm text-green-700">Topic ID: {item.topic_id} • Fila: {item.row_number}</div>
          ...
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2 text-red-700">
                ...
                Tópicos con Errores ({result.failed_count})
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3 max-h-48 overflow-y-auto">
                  ...
                  <div key={index} className="p-3 bg-red-50 border border-red-200 rounded-lg">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <div className="text-sm text-red-700">Topic ID: {item.topic_id} • Fila: {item.row_number}</div>
                      </div>
        ...
        <div className={`p-4 rounded-lg border ${successRate === 100 ? 'bg-green-50 border-green-200' : successRate >= 50 ? 'bg-yellow-50 border-yellow-200' : 'bg-red-50 border-red-200'}`}>
          ...
          <p className={`text-sm mt-1 ${successRate === 100 ? 'text-green-700' : successRate >= 50 ? 'text-yellow-700' : 'text-red-700'}`}>
            {result.successful_count} de {result.total_processed} tópicos fueron importados correctamente.
