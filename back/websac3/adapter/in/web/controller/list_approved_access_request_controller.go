...
type ListApprovedAccessRequestController struct{ Authenticable }

var listApprovedAccessRequestControllerInstance *ListApprovedAccessRequestController = nil

func GetListApprovedAccessRequestController() *ListApprovedAccessRequestController {
	if listApprovedAccessRequestControllerInstance == nil {
		listApprovedAccessRequestControllerInstance = &ListApprovedAccessRequestController{}
	}
	return listApprovedAccessRequestControllerInstance
}

// Handle lista solicitudes de acceso aprobadas con paginación y filtros
// @Summary Listar Solicitudes de Acceso Aprobadas
// @Description Lista las solicitudes de acceso aprobadas existentes según filtros dinámicos y parámetros de paginación.
...
// @Description - `ApprovedRole.name[eq|cont]`
// @Description
...
// @Success 200 {object} response.ApiResponse[paginator.Page[response.ListApprovedAccessRequestResponse]] "Se obtuvieron las solicitudes de acceso aprobadas exitosamente"
...
// @Failure 404 {object} response.ApiResponse[string] "No se encontraron solicitudes de acceso aprobadas"
// @Failure 500 {object} response.ApiResponse[string] "Error interno del servidor"
// @Router /api/v1/{lang}/access-requests/approved [get]
// @Security BearerAuth
func (c *ListApprovedAccessRequestController) Handle(ctx *gin.Context) {
	var err error
	...
	var requestQuery = query.ListApprovedAccessRequestQuery{
	...
	result, _ := mediator.Send[query.ListApprovedAccessRequestQuery, response.ApiResponse[paginator.Page[response.ListApprovedAccessRequestResponse]]](requestQuery, lang)
