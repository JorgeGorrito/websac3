...
var listRoleControllerInstance *ListRoleController = nil

func GetListRoleController() *ListRoleController {
	if listRoleControllerInstance == nil {
		listRoleControllerInstance = &ListRoleController{}
	}
	return listRoleControllerInstance
}

// Handle lista los roles del sistema con paginación y filtros
// @Summary Listar Roles del Sistema
// @Description Lista los roles del sistema existentes según filtros dinámicos y parámetros de paginación.
// @Description Los filtros deben enviarse como query params con formato: `campo[operador]=valor`. Por ejemplo: `name[cont]=admin`.
// @Description
...
// @Param filters query string false "Filtros dinámicos: formato campo[operador]=valor (ej: name[cont]=admin)"
// @Param lang path string true "Código de idioma" default(en) Enums(en, es)
// @Success 200 {object} response.ApiResponse[paginator.Page[response.ListRoleResponse]] "Se obtuvieron los roles del sistema exitosamente"
// @Failure 400 {object} response.ApiResponse[string] "Formato de solicitud inválido (ejemplo: JSON mal formado)"
// @Failure 404 {object} response.ApiResponse[string] "No se encontraron roles del sistema"
// @Failure 500 {object} response.ApiResponse[string] "Error interno del servidor"
// @Router /api/v1/{lang}/roles [get]
func (c *ListRoleController) Handle(ctx *gin.Context) {
	var err error
	...
	var requestQuery = query.ListRoleQuery{
		PaginationParams: paginationParams,
		Filters:          filters,
	}

	result, _ := mediator.Send[query.ListRoleQuery, response.ApiResponse[paginator.Page[response.ListRoleResponse]]](requestQuery, lang)
