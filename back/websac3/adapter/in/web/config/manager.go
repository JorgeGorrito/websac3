...
func (m *manager) ConfigureMediator(errorList *error) {
	...
	if err := iMediator.Register(
		reflect.TypeOf(command.DeleteDegreeProgramCommand{}),
		chandler.NewDeleteDegreeProgramCommandHandler(
			container.Inject[usecase.DeleteDegreeProgramUseCase](),
			container.Inject[message.Provider](),
			container.Inject[logging.Logger](),
			container.Inject[validator.Validator](),
		),
	...
	if err := iMediator.Register(
		reflect.TypeOf(command.BulkCreateCourseCommand{}),
		chandler.NewBulkCreateCourseCommandHandler(
			container.Inject[usecase.BulkCreateCourseUseCase](),
			container.Inject[validator.Validator](),
			container.Inject[message.Provider](),
			container.Inject[logging.Logger](),
		),
	...
	if err := iMediator.Register(
		reflect.TypeOf(query.ListAnsweredExpertConsultationsQuery{}),
		qhandler.NewListAnsweredExpertConsultationsQueryHandler(
			container.Inject[validator.Validator](),
			container.Inject[logging.Logger](),
			container.Inject[usecase.ListAnsweredExpertConsultationsUseCase](),
			container.Inject[message.Provider](),
		),
	); err != nil {
		*errorList = errors.Join(*errorList, err)
