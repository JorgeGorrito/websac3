...
type threatHunterRole struct{}

func ThreatHunterRole() Seeder { return &threatHunterRole{} }

func (s *threatHunterRole) Seed(ctx _db.Context) error {
	dbCtx, ok := ctx.(*db.Context)
	if !ok {
		return errors.New("invalid db context type")
	}

	// 1) Create/find ProfessionalRole: Threat Hunter
	var role model.ProfessionalRole
	if err := dbCtx.DB().Where("name = ?", "Threat Hunter").First(&role).Error; err != nil {
		// Try to create if not exists
		role = model.ProfessionalRole{Name: "Threat Hunter"}
	...
	// 2) Define KA expected for Threat Hunter (weights sum to 1.0)
	...
	cfg := []kaCfg{
		{
			Name: "Connection Security", Weight: 0.35,
			Topics: []topicCfg{
				{Name: "Network Defense - Threat Hunting and Machine Learning", Hours: 12},
		...
		},
		{
			...
			Topics: []topicCfg{
				...
				{Name: "Personal Data Privacy and Security - Sensitive Personal Data (SPD)", Hours: 6},
		...
		},
		{
			...
			Topics: []topicCfg{
				...
				{Name: "Analytical Tools - Data Analytics", Hours: 8},
				{Name: "Analytical Tools - Security Intelligence", Hours: 8},
	...
	// 4) Assign Threat Hunter role to all existing degree programs
